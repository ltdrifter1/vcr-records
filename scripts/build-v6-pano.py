#!/usr/bin/env python3
"""
V6 panorama production bake — polished 90s cartoon / skate-shop pass.

Input : art/vcr-pano-v6-src.png  (1536x1024 clean cel-shaded ink painting,
        wrap-aware edges, VCR RECORD SHOP lettering, sticker-slapped counter)
Output: public/textures/store_pano_v6.webp        4096x2048 lights-on
        public/textures/store_pano_off_v6.webp    4096x2048 lights-off grade
        public/textures/store_pano_lqip_v6.webp   512x256 preview
        public/hotspots/<id>_edge.webp            silhouette rim masks

The source keeps V5's composition exactly (same landmark boxes, same wrap
edges on plain wall) so hotspot UVs in app/data/sections.ts stay valid.

Run: python3 scripts/build-v6-pano.py
"""
...
SRC = ROOT / "art" / "vcr-pano-v6-src.png"
OUT_ON = ROOT / "public" / "textures" / "store_pano_v6.webp"
OUT_OFF = ROOT / "public" / "textures" / "store_pano_off_v6.webp"
OUT_LQIP = ROOT / "public" / "textures" / "store_pano_lqip_v6.webp"
HOTSPOT_DIR = ROOT / "public" / "hotspots"

W, H = 4096, 2048
SEAM_BAND = 128  # px cross-faded across the wrap seam

# Landmark boxes in source pixels (1536x1024) — unchanged from V5 so the
# authored hotspot UVs keep pointing at the same furniture.
TARGETS = {
...
def lights_off(pano: Image.Image) -> Image.Image:
    ...
    # Lamp pools (file-space u/v fractions of the V6 source painting).
    for fu, fv, radius, gain in (
        (0.386, 0.26, 0.15, 0.5),
        (0.435, 0.315, 0.13, 0.45),
        (0.545, 0.36, 0.13, 0.45),
        (0.645, 0.23, 0.16, 0.55),
        (0.823, 0.40, 0.18, 0.35),  # door window spill
    ):
