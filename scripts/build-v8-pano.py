#!/usr/bin/env python3
"""
V8 panorama production bake — warm earth-tone premium pass.

Input : art/vcr-pano-v8-src.png  (1536x1024 cel painting: sandy clay walls,
        cedar beams, oak floor, rich natural-wood fixtures, clean counter
        with no stickers / lettering — modern West Coast + 90s nostalgia)
Output: public/textures/store_pano_v8.webp        4096x2048 lights-on
        public/textures/store_pano_off_v8.webp    4096x2048 lights-off grade
        public/textures/store_pano_lqip_v8.webp   512x256 preview
...
Run: python3 scripts/build-v8-pano.py
"""
...
SRC = ROOT / "art" / "vcr-pano-v8-src.png"
OUT_ON = ROOT / "public" / "textures" / "store_pano_v8.webp"
OUT_OFF = ROOT / "public" / "textures" / "store_pano_off_v8.webp"
OUT_LQIP = ROOT / "public" / "textures" / "store_pano_lqip_v8.webp"
...
PLANES = {
    ...
    "crt-tv": (0.8766, 0.5366, 26.0, 14.0),
    "record-bins": (0.482, 0.59, 26.5, 32.5),
    "cash-register": (0.323, 0.48, 26.5, 20.5),
    "phone-booth": (0.227, 0.53, 20.0, 10.5),
}
# CRT hit plane (sections.ts w/h) — tube-locked for the video overlay stack.
CRT_PLANE = (0.8766, 0.5366, 20.9, 11.3)
...
SEG = {
    ...
    "cash-register": (110, 8, True),
    "phone-booth": (110, 16, True),
}

# Background color sample points (source-px of the 1536x1024 painting) used
# by the color-refine pass: clay wall + wood counter/cabinet/floor fills.
BG_POINTS = ((1300, 340), (60, 340), (990, 550), (1100, 650), (150, 660), (890, 780))

# Lamp pools for the lights-off grade (file-space u/v of the V8 source).
LAMP_POOLS = (
    (0.210, 0.124, 0.12, 0.5),
    (0.231, 0.179, 0.11, 0.45),
    (0.303, 0.176, 0.12, 0.5),
    (0.345, 0.109, 0.12, 0.45),
    (0.501, 0.088, 0.16, 0.4),  # fan cluster
    (0.923, 0.45, 0.16, 0.4),  # door window daylight spill
)
