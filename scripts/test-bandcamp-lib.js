...
const selfTitled = catalog.releases.find((r) => r.id === "l-t-drifta");
...
assert.ok(selfTitled);
assert.strictEqual(selfTitled.catalogue, "CC025");
assert.strictEqual(selfTitled.previewTrackId, "ltd-03");
assert.strictEqual(selfTitled.formats.digital.sku, "dg-l-t-drifta");
assert.strictEqual(selfTitled.formats.digital.price, 6.99);
assert.ok(/ltdrifta\.bandcamp\.com\/album\/l-t-drifta/.test(selfTitled.bandcampUrl));
assert.strictEqual(bc.pickCueTrack(selfTitled).id, "ltd-03");
assert.strictEqual(selfTitled.tracks[2].title, "Need U Close 2 Me");
assert.strictEqual(selfTitled.tracks[2].bandcampTrackId, 2859245917);
